...
function computerMove(game) {
 const positions = [
    ...
    const move = {outerPosition: positions[rand()], innerPosition: positions[rand()]}
    ...
    if (illegalBoard || boardComplete || illegalMove) {
      console.log("Computer chose a move that was not allowed", move, game.boardState[move.outerPosition][move.innerPosition]);
